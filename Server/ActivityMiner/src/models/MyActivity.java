...
public class MyActivity {
    ...
    public static final double MIN_TRACE_POINTS_IN_ACTIVITY = 60;
...
//        trace = records;
        ...
        for (LatLng latLng : points) {
            totalDistance += GeometryUtil.calculateLineDistance(latLng, area.center);
        }
        averageCenterDistance = totalDistance / points.size();
        averageVectorProduct /= points.size();
        double target = (double)trace.size() * ASSURANCE_OUTDOOR_THRESHOLD;
...
//                "\n平均向量内积: " + averageVectorProduct;
        return formatter.format(getStartDate()) + " -- " + formatter.format(getEndDate()) + ", " + activityType + ", " +
                trace.size() + "条记录";
    }
    ...
    public static ArrayList<MyActivity> splitActivities(ArrayList<Record> records, int duration) {
        ...
        for (Record record: records) {
            if (record.getTimepoint().getTime() <= endDate.getTime()) {
                ...
                if (list.size() >= MIN_TRACE_POINTS_IN_ACTIVITY) {
        ...
        for (MyActivity activity : activities) {
            ...
            merged = mergeActivity(lastActivity, activity);
            if (!merged) {
                mergedActivities.add(lastActivity);
                lastActivity = activity;
            } else {
                lastActivity = new MyActivity(lastActivity.getTrace());
            }
        }
        if (lastActivity != null)
            mergedActivities.add(lastActivity);
